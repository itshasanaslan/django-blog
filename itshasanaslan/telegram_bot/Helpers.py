...
class TelegramControl:
    def __init__(self, token, drive_api, quiz_controller):
        ...
        self.commands = {
            "download" : "/drive",
            "get_urls":"/url", "get_scores":"/skor"
        }
        self.help_commands = {
        "/drive" : "Drive'a dosya yükler. Bir dosya mesajını alıntıla ve şöyle yaz: /drive klasöradı",
        "/url" :  "Drive klasöründen dosyaları ve linklerini getirir. Kullanımı: /url klasöradı", "/skor":"quiz skor tablosu."
    }
    ...
    def command_manager(self, update):
        ...
        elif commands[0] == self.commands['get_scores']:update.message.reply_text(self.quiz_controller.get_scores())
...
###############################################################################################
    ...
    def download_file(self,filename, file_id = None, file_path = None):
        ...
        print(download_url)
        downloaded_path = os.path.join(self.data_folder, self.turkify(filename)) 
        #downloaded_path = os.path.join(self.data_folder,filename) test
    ...
    def turkify(self, name):
        s = ""
        data = {
            'ö':'o', 'ı':'i', 'ş': 's', 'ü' : 'u', 'ğ' : 'g', 'ç' : 'c',
            'Ö':'O', 'İ':'I', 'Ş': 'S', 'Ü' : 'U', 'Ğ' : 'G', 'Ç' : 'C'
        }
        for i in name:
            if i in data:
                s += data.get(i)
...
class QuizHandler:
    def __init__(self):
        ...
        #self.telegram_data_folder = "telegram_data"
    ...
    def alert(self, user, inner = False):
        ...
        msg += self.get_scores()
        msg += f"\n\n\nEn çok katkıda bulunan '{max_sender}' kişisine teşekkürler :)"
        return msg
    ...
    def read_data(self):
        ...
        #for item,key in self.quiz_senders_info.items():
            #print(item, key)
        cursor.close()
    ...
    def get_scores(self):
        m = ""
        users_arr = []
        for i,j in self.quiz_senders_info.items(): 
            users_arr.append([i,j])
        counter = 1
        users_arr = self.sort_user(users_arr, reverse =  True)
        for i in users_arr:
            m += str(counter) + ") " + str(i[1][0]).title() + " : " + str(i[1][1]) + "\n" + "-" * 20 + "\n"
            counter += 1
        return m

    def sort_user(self, arr, reverse = False):
        n = len(arr)
        
        for i in range(n-1): 
            for j in range(0, n-i-1): 
                if arr[j][1][1] > arr[j+1][1][1] : 
